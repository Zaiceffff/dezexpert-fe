...
export function TestDataManager({ onDataCreated }: TestDataManagerProps) {
  ...
  const handleCreateTestData = async () => {
    try {
      ...
      const response = await fetch(`${process.env.AVITO_BACKEND_URL || 'https://144.124.230.138:3005'}/api/avito/listings/seed`, {
        method: 'POST',
        headers: {
  ...
  const handleFetchRealData = async () => {
    try {
      ...
      const response = await fetch(`${process.env.AVITO_BACKEND_URL || 'https://144.124.230.138:3005'}/api/avito/listings/fetch-real`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
