...
export default function AvitoCallbackPage() {
  ...
  const handleOAuthCallback = async (code: string, state?: string) => {
    try {
      ...
      const response = await fetch(`${process.env.AVITO_BACKEND_URL || 'https://v467850.hosted-by-vdsina.com/api/docs'}/api/avito/oauth/callback/public`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
        },
        // Передаем параметры через URL
      });
