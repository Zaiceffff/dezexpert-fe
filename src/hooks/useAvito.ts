...
export function useAvito(): UseAvitoReturn {
  ...
  const getOAuthUrl = useCallback(async (): Promise<AvitoOAuthUrlResponse> => {
    try {
      ...
      const response = await fetch(`${process.env.AVITO_BACKEND_URL || 'https://v467850.hosted-by-vdsina.com/api/docs'}/api/avito/oauth/url`, {
        method: 'GET',
  ...
  const connectAccount = useCallback(async (code: string, state?: string): Promise<boolean> => {
    try {
      ...
      const response = await fetch(`${process.env.AVITO_BACKEND_URL || 'https://v467850.hosted-by-vdsina.com/api/docs'}/api/avito/oauth/callback?${params}`, {
        method: 'GET',
        headers: getAuthHeaders(),
      });
  ...
  const syncListings = useCallback(async (force = false): Promise<boolean> => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.AVITO_BACKEND_URL || 'https://v467850.hosted-by-vdsina.com/api/docs'}/api/avito/listings`, {
        method: 'POST',
        headers: getAuthHeaders(),
  ...
  const toggleAiAssistant = useCallback(async (listingId: string, enabled: boolean): Promise<boolean> => {
    try {
      ...
      const response = await fetch(`${process.env.AVITO_BACKEND_URL || 'https://v467850.hosted-by-vdsina.com/api/docs'}/api/avito/listings/${listingId}/ai-toggle`, {
        method: 'POST',
        headers: getAuthHeaders(),
  ...
  const getActiveListings = useCallback(async (): Promise<boolean> => {
    try {
      ...
      const response = await fetch(`${process.env.AVITO_BACKEND_URL || 'https://v467850.hosted-by-vdsina.com/api/docs'}/api/avito/listings/ai-active`, {
        method: 'GET',
        headers: getAuthHeaders(),
      });
